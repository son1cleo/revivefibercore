...
export async function getAllAdminImpactStats() {
  const supabase = await createSupabaseServerClient();
  ...
  const { data, error } = await supabase.from("impact_stats").select("*").order("year", { ascending: false });
...
export async function getAdminImpactStatById(id) {
  const supabase = await createSupabaseServerClient();
  ...
  const { data, error } = await supabase.from("impact_stats").select("*").eq("id", id).maybeSingle();
