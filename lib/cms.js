...
function normalizeBlogRow(row) {
  return {
    ...
    author: row.author || "Revive Fiber Co Team",
    excerpt: row.excerpt || "",
    content: row.content || "",
...
export async function getAllAdminEmploymentApplications() {
  const supabase = await createSupabaseServerClient();
  ...
  const { data, error } = await supabase.from("employment_applications").select("*").order("created_at", { ascending: false });
...
export async function getAdminEmploymentApplicationById(id) {
  const supabase = await createSupabaseServerClient();
  ...
  const { data, error } = await supabase.from("employment_applications").select("*").eq("id", id).maybeSingle();
